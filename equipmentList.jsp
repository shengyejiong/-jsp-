...
<script type="text/javascript">
    ...
    function deletel(obj){
        ...
        form.method = "post";
        form.action = "EquipmentDeleteServlet";
...
//添加数据函数
        ...
        f.action="EquipmentAddServlet"
        f.submit();

}
    function update(){
        var f = document.forms[0];
        f.action="EquipmentEditServlet"
        f.submit();
    }
...
<%//按空格切割成数组
    ...
    int b = 5;
    int c = 0;
    List<Company> companyList = new ArrayList<>();//批量创建javabean对象
    for(int d=0;d<length/8;d++){
        Company number = new Company();
        number.setDesname(inputs[c++]);
        number.setMacname(inputs[c++]);
        number.setMaccode(inputs[c++]);
        number.setMacvary(inputs[c++]);
        number.setMactype(inputs[c++]);
        number.setInstalltime(inputs[c++]+" "+inputs[c++]);
        number.setMacstate(inputs[c++]);
        companyList.add(number);
...
<%--创建的javabean对象输出--%>
    <%for (Company company:companyList){%>
        <tr>
              <td><%=company.getDesname()%></td>
              <td><%=company.getMacname()%></td>
              <td><%=company.getMaccode()%></td>
              <td><%=company.getMacvary()%></td>
              <td><%=company.getMactype()%></td>
              <td><%=company.getInstalltime()%></td>
              <td><%=company.getMacstate()%></td>
      ...
      <form name="table2"  method="post">
          ...
          <tr>
          <th>站点名称(*)</th>
          <select name="company" id="portname">
          ...
          <tr>
          <th>设备名称(*)</th>
          <td><input type="text" id="devicename" name="device-name"/></td>
          <br>
          </tr>
        
          <tr>
          <td>设备编码(*)</td>
          <td><input type="text" id="num" name="device-code"/></td>
          <br></tr>

          <tr>
          <td>设备分类</td>
          <select name="selection" id="vary" >
          ...
          <tr>
          <td>安装时间</td>
          <td><input type="text" id="installtime" name="install time"></td>
          </tr><br>

          <tr>
          <td>设备状态</td>
          <select name="state" id="now">
